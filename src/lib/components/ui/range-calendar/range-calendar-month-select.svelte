...
<span
  class={cn(
    `
      relative flex rounded-md border border-input shadow-xs
      has-focus:border-ring has-focus:ring-[3px] has-focus:ring-ring/50
    `,
    className,
  )}
>
  <RangeCalendarPrimitive.MonthSelect bind:ref class='
  ...
  ' {...restProps}>
    {#snippet child({ props, monthItems, selectedMonthItem })}
      ...
      <span
        class='
          flex h-8 items-center gap-1 rounded-md pr-1 pl-2 text-sm font-medium
          select-none
